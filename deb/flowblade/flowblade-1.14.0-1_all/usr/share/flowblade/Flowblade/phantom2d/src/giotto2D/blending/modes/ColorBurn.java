...
public class ColorBurn extends AbstractBlender
{
		...
		for( i = xStart; i < xEnd; i++ )
		{
			for( j = yStart; j < yEnd; j++ )
			{
				...
				if( sPixel[ RED ] == 0 )
					...
					rVal = 255 - ( 255 - dPixel[ RED ] ) * 255 / sPixel[ RED ];
				if( rVal < 0 ) rVal = 0;

				if( sPixel[ GREEN ] == 0 )
					gVal = 0;
				else 
					gVal = 255 - ( 255 - dPixel[ GREEN ] ) * 255 / sPixel[ GREEN ];
				if( gVal < 0 ) gVal = 0;

				if( sPixel[ BLUE ] == 0 )
					bVal = 0;
				else 
					bVal = 255 - ( 255 - dPixel[ BLUE ] ) * 255 / sPixel[ BLUE ];
				if( bVal < 0 ) bVal = 0;
