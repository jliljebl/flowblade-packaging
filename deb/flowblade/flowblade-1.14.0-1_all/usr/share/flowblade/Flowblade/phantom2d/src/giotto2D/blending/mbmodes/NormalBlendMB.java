...
//--- Special motion blur normal blend.
public class NormalBlendMB extends AbstractBlender
{
				...
				Rectangle blendArea )
	{
		...
		//--- Channel values, aplha values, and optimizing cube
		int rVal, gVal, bVal;
		int aS, aD;
		int i, j;
		...
		for( i = xStart; i < xEnd; i++ )
		{
			...
			{
				//--- Get blend mask pixel.
				//--- Check if blend done for this pixel. Black in mask means no.
				...
				//--- Calculate source and destination alphas.
				aS = bmPixel[ RED ]  * intOpacity / 255;
				aD = 255 - aS;
				
				//--- Do blend. Set destination pixel values using normal blend rule.
...
/*
				rVal = (sPixel[ RED ] * cube )  / ( bmPixel[ RED ] * 255 );
				if( rVal > 255 ) rVal = 255;

				gVal = (sPixel[GREEN] * cube )  / ( bmPixel[ RED ] * 255);
				if( gVal > 255 ) gVal = 255;

				bVal = (sPixel[ BLUE ] * cube )  / ( bmPixel[ RED ] * 255);
				if( bVal> 255 ) bVal = 255;
*/