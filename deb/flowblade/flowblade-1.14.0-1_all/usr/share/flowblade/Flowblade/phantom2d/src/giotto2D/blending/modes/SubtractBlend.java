...
public class SubtractBlend extends AbstractBlender
{
				...
				Rectangle blendArea )
	{
		...
		for( i = xStart; i < xEnd; i++ )
		{
			for( j = yStart; j < yEnd; j++ )
			{
				...
				//--- Do blend
				///a + b - 1, in normalized space
				rVal = dPixel[ RED ] - sPixel[ RED ] ;
				if( rVal < 0 ) rVal = 0; 

				gVal = dPixel[ GREEN ] - sPixel[ GREEN ];
				if( gVal < 0 ) gVal = 0;
	
				bVal = dPixel[ BLUE ] - sPixel[ BLUE ];
				if( bVal < 0 ) bVal = 0;
